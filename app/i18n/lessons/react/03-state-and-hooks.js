  ...
  ar: {
    sections: [
      ...
      { title: "لماذا نحتاج إلى الحالة؟", content: [
        { type: "p", text: "بدون الحالة، لا يمكنك بناء تطبيقات تفاعلية. تخيل أن كل شيء في واجهة المستخدم كان ثابتاً ولم يتغير أبداً." },
        { type: "li", text: "لن يتزايد العداد عند النقر على زر" },
        { type: "li", text: "لن يتغير قائمة المنتجات عند تحديد فئة" },
        { type: "li", text: "لن يعمل التنقل بين الصفحات" },
        { type: "li", text: "لن تحتفظ النماذج بالبيانات المدخلة" },
        { type: "p", text: "الحالة هي ما يجعل تطبيقات React <strong>تفاعلية</strong>." },
      ]},
      { title: "خطاف useState", content: [
        { type: "p", text: "<code>useState</code> هو الخطاف الأول الذي ستستخدمه. يسمح لك بـ \"تثبيت\" ميزات الحالة في مكون الدالة." },
        { type: "p", text: "الصيغة الأساسية لـ <code>useState</code>:" },
        { type: "li", text: "<strong>العنصر الأول:</strong> قيمة الحالة الحالية (currentState)" },
        { type: "li", text: "<strong>العنصر الثاني:</strong> دالة لتحديث القيمة (setStateFunction)" },
        { type: "callout", title: "مهم", text: "عادة ما يبدأ اسم دالة التحديث بـ <code>set</code> مثل <code>setCount</code> و <code>setName</code>. هذه اتفاقية وليست قاعدة صارمة." },
      ]},
      { title: "مثال بسيط - العداد", content: [
        { type: "p", text: "إليك عداداً بسيطاً باستخدام <code>useState</code>:" },
        { type: "li", text: "أنشئ حالة بقيمة أولية <code>0</code> باستخدام <code>useState(0)</code>" },
        { type: "li", text: "احصل على <code>count</code> (القيمة الحالية) و <code>setCount</code> (دالة التحديث)" },
        { type: "li", text: "عند النقر على زر \"زيادة\"، استدعِ <code>setCount(count + 1)</code>" },
        { type: "li", text: "يعيد React العرض بالقيمة الجديدة تلقائياً" },
      ]},
      { title: "مثال عملي - قائمة المهام", content: [
        { type: "p", text: "مثال أكثر تعقيداً: قائمة مهام تسمح بإضافة وإكمال وحذف المهام." },
        { type: "li", text: "استخدام <code>useState</code> بمصفوفة كقيمة أولية" },
        { type: "li", text: "استخدام <code>setTodos</code> مع <code>map</code> و <code>filter</code>" },
        { type: "li", text: "استخدام عامل الانتشار <code>[...todos, newTodo]</code>" },
        { type: "li", text: "إدارة قيمتي حالة في نفس المكون" },
      ]},
      { title: "قواعد الخطافات", content: [
        { type: "p", text: "هناك قواعد صارمة عند استخدام الخطافات في React:" },
        { type: "li", text: "<strong>لا</strong> تستدعي الخطافات داخل الحلقات أو الشروط أو الدوال المتداخلة" },
        { type: "li", text: "<strong>لا</strong> تستدعي الخطافات داخل الدوال العادية" },
        { type: "li", text: "<strong>نعم</strong> تستدعي الخطافات في المستوى الأعلى من مكونك" },
        { type: "li", text: "<strong>نعم</strong> تستدعي الخطافات من مكونات الدوال في React" },
      ]},
      ...
      { title: "ماذا يحدث خلف الكواليس؟", content: [
        { type: "p", text: "عندما تستدعي setter مثل <code>setCount</code)، يجدول React تحديث الحالة ويعيد عرض المكون." },
        { type: "li", text: "<strong>تحديث الحالة:</strong> يجدول React القيمة الجديدة (قد لا يحدث فوراً)" },
        { type: "li", text: "<strong>جدولة إعادة العرض:</strong> يعيد React عرض المكون والمكونات الفرعية في دورة تحديث واحدة" },
        { type: "li", text: "<strong>إعادة العرض:</strong> تُستدعى دالة المكون مرة أخرى بالحالة الجديدة" },
        { type: "li", text: "<strong>مقارنة DOM الافتراضي:</strong> يقارن React شجرة DOM الافتراضية الجديدة مع القديمة" },
        { type: "li", text: "<strong>تحديث DOM الحقيقي:</strong> يتم تحديث الأجزاء المتغيرة فقط في الصفحة الحقيقية" },
      ]},
    ],
    quiz: [
      { question: "ما هي الطريقة الصحيحة لإنشاء الحالة باستخدام useState؟", options: ["const state = useState(0)", "const [count, setCount] = useState(0)", "const { count, setCount } = useState(0)", "let count = useState(0)"], correctAnswer: 1, explanation: "يُرجع useState مصفوفة بعنصرين. نستخدم التفكيك لفصلهما: القيمة الحالية ودالة التحديث." },
      { question: "ماذا يحدث عند استدعاء دالة set في React؟", options: ["تتحدث الحالة فوراً وتعيد العرض", "تتحدث الحالة وتعيد العرض في الدورة التالية", "تتحدث الحالة فقط بدون إعادة العرض", "تعيد العرض فقط بدون تغيير الحالة"], correctAnswer: 1, explanation: "الحالة لا تتحدث فوراً. يحدث التحديث في دورة العرض التالية." },
      { question: "ما الفرق بين الخصائص والحالة؟", options: ["الخصائص للقراءة/الكتابة، الحالة للقراءة فقط", "الخصائص من الأصل (خارجية)، الحالة داخل المكون (داخلية)", "الخصائص للنصوص فقط، الحالة للأرقام فقط", "لا يوجد فرق بينهما"], correctAnswer: 1, explanation: "الخصائص تأتي من المكون الأصل ولا يمكن تغييرها (للقراءة فقط). الحالة داخلية ويمكن تغييرها بدالة Setter." },
      { question: "أي استخدام صحيح لـ useState؟", options: ["داخل حلقة for", "داخل شرط if", "في المستوى الأعلى من المكون فقط", "داخل دالة متداخلة"], correctAnswer: 2, explanation: "يجب استدعاء الخطافات في المستوى الأعلى فقط، وليس في الحلقات أو الشروط أو الدوال المتداخلة." },
      { question: "لماذا نستخدم دالة التحديث بدلاً من القيمة المباشرة؟", options: ["React لا يدعم التحديثات المباشرة", "أسهل للكتابة", "يضمن التحديثات الصحيحة عند البناء على الحالة السابقة", "أسرع في الأداء"], correctAnswer: 2, explanation: "تدعم دالة التحديث القيمة الأخيرة للحالة حتى مع التحديثات السريعة." },
    ],
    challenge: { title: "ابنِ عداداً ذكياً مع مفتاح", description: "أنشئ تطبيقاً يجمع بين عداد ومفتاح إظهار/إخفاء المحتوى باستخدام الحالة." },
    cheatSheet: { title: "ملخص الحالة والخطافات", columns: [
