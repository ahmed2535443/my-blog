const translations = {
  ar: {
    sections: [
      { title: "مقدمة في لوحة التحكم والإدارة", content: [
        { type: "p", text: "لوحة التحكم هي مركز التحكم لإدارة محتوى المدونة. في هذا الدرس، نبني لوحة تحكم كاملة مع إدارة المقالات وإدارة التصنيفات ورفع الصور." },
        { type: "callout", title: "ما سنبنيه", text: "الصفحة الرئيسية للوحة التحكم مع الإحصائيات، إدارة المقالات بعمليات CRUD، إدارة التصنيفات، رفع الصور باستخدام Supabase Storage، التقسيم لمجموعات البيانات الكبيرة." }
      ]},
      { title: "الصفحة الرئيسية للوحة التحكم", content: [
        { type: "p", text: " تعرض لوحة التحكم الإحصائيات (إجمالي المقالات والتصنيفات والمستخدمين) والمقالات الأخيرة. استخدم Promise.all لجلب عدادات متعددة بالتوازي." },
        { type: "callout", title: "نصيحة الأداء", text: "Promise.all ينفذ استعلامات قاعدة البيانات المتعددة في نفس الوقت بدلاً من التسلسلية، مما يحسن أوقات التحميل بشكل كبير." }
      ]},
      { title: "إدارة المقالات", content: [
        { type: "p", text: " إدارة جميع المقالات بعرض جدول يعرض العنوان والتصنيف والحالة والتاريخ وأزرار الإجراءات للتعديل وتبديل النشر والحذف." },
        { type: "callout", title: "إجراءات المقالات", text: "التعديل يفتح نموذج التعديل، تبديل النشر يغير الحالة بين المسودة والمنشور، الحذف يزيل المقال مع التأكيد." }
      ]},
      { title: "رفع الصور باستخدام Supabase Storage", content: [
        { type: "p", text: "ارفع الصور إلى Supabase Storage مع التحقق من نوع الملف وحدود الحجم وتوليد رابط تلقائي." },
        { type: "callout", title: "أفضل ممارسات الرفع", text: "تحقق من أنواع الملفات (JPEG، PNG، WebP)، فرض حدود الحجم (5 ميجابايت)، توليد أسماء ملفات فريدة، تقديم معاينة الرفع." }
      ]},
      { title: "إدارة التصنيفات", content: [
        { type: "p", text: "إنشاء وتعديل وحذف التصنيفات. عرض عدد المقالات لكل تصنيف." },
        { type: "callout", title: "عمليات التصنيفات", text: "نموذج مضمن للإنشاء/التعديل، الحذف مع التأكيد، عرض عدد المقالات لكل تصنيف." }
      ]},
      { title: "التقسيم", content: [
        { type: "p", text: "يقسم التقسيم مجموعات البيانات الكبيرة إلى صفحات لأفضل أداء وتجربة مستخدم." },
        { type: "callout", title: "تنفيذ التقسيم", text: "احسب الإزاحة من الصفحة الحالية، استخدم Supabase .range() لتحديد النتائج، اعرض أرقام الصفحات معنقطة للكثير من الصفحات." }
      ]}
    ],
    quiz: [
      { question: "لماذا نستخدم Promise.all لجلب إحصائيات لوحة التحكم؟",
        options: ["مطلوب من JavaScript", "ينفذ استعلامات متعددة بالتوازي لأفضل أداء", "هو الطريقة الوحيدة للاستعلام من Supabase", "يمنع الأخطاء"],
        correctAnswer: 1, explanation: "يقوم Promise.all بتشغيل عمليات غير متزامنة متعددة في نفس الوقت، وهو أسرع بكثير من تشغيلها واحدًا تلو الآخر." },
      { question: "ما هو الهدف من التقسيم؟",
        options: ["جعل قاعدة البيانات أصغر", "تقسيم مجموعات البيانات الكبيرة إلى صفحات قابلة للإدارة", "منع حقن SQL", "تشفير البيانات"],
        correctAnswer: 1, explanation: "يقسم التقسيم مجموعات النتائج الكبيرة إلى صفحات أصغر، مما يحسن الأداء وتجربة المستخدم." }
    ],
    challenge: { title: "تحدي: إضافة البحث إلى لوحة التحكم",
      description: "أضف حقل بحث إلى صفحة إدارة المقالات. أنشئ مكون SearchInput مع debounce، استخدم معلمات URL لاستعلام البحث، نفذ تصفية Supabase باستخدام .ilike()." },
    cheatSheet: { title: "مرجع لوحة التحكم السريع", items: [
      { title: "Supabase Storage", content: ".from('bucket').upload(path, file)، .getPublicUrl(path)" },
      { title: "التقسيم", content: ".range(start, end)، { count: 'exact' }، حساب الإزاحة" },
      { title: "الأداء", content: "Promise.all للاستعلامات المتوازية، useOptimistic لتحديثات واجهة المستخدم" }
    ]}
  },
  en: {
    sections: [
