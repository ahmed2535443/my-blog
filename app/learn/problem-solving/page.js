...
export default function ProblemSolvingStagePage() {
  const stage = getStageById("problem-solving");
    ...
    <div className="container mx-auto px-4 py-12">
      <header className="mb-10">
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-4xl font-extrabold" style={{ color: "var(--foreground)" }}>{stage.title}</h1>
            <p className="text-lg" style={{ color: "var(--muted)" }}>{stage.description}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {stage.lessons.map((lesson, index) => (
            ...
            >
              ...
              <span className="text-lg transition-transform duration-300 group-hover:-translate-x-2 shrink-0" style={{ color: "var(--muted)" }}>←</span>
            </Link>
          ))}
        </div>
      </div>
